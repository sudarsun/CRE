...
int QuadProg(const Qdata &q, double *c, int classes, real_array &outValues) {
	...
	auto *bkx = new MSKboundkeye[classes];
	auto *blx = new double[classes],
			*bux = new double[classes];

	auto *aptrb = new MSKint32t[classes],
			*aptre = new MSKint32t[classes],
			*asub = new MSKint32t[classes];
	auto *aval = new double[classes];

	// automatic memory release
	std::unique_ptr<MSKint32t> j1(aptrb), j2(aptre), j3(asub);
	std::unique_ptr<double> j4(aval), j5(blx), j6(bux);
	std::unique_ptr<MSKboundkeye> j7(bkx);
	...
	if (r == MSK_RES_OK) {
		...
		if (r == MSK_RES_OK) {
			...
			if (r == MSK_RES_OK) {
				...
				{
					boost::lock_guard<boost::mutex> guard(gMTX);
					/* Run optimizer */
					...
					switch (solsta) {
						...
						case MSK_SOL_STA_NEAR_OPTIMAL:
							...
							//fprintf(stderr, "Optimal primal solution\n");
							{
								double denom = 0.0f;
								for (j = 0; j < NUMVAR; ++j) {
									// if the value goes more than 1.0, reset it to 1.0
									if (xx[j] > 1.0)
										xx[j] = 1.0;

									// round off 1.0+\eps to 1.0
									if (fabs(xx[j] - 1.0) < 1E-5)
										xx[j] = 1.0;

									// round off 0.0+\eps to 0.0
									if (fabs(xx[j]) < 1E-5)
										xx[j] = 0.0;
								...
								// normalize the class proportions probability.
								for (j = 0; j < NUMVAR; ++j) {
									//fprintf(stderr, "x[%d]: %e\n",j,xx[j]);
									outValues[j] = xx[j] / denom;
								}
							}
							break;
